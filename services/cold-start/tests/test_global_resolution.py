...
async def test_fragment_candidates_are_recalled_without_auto_identity() -> None:
    existing = global_object(
        ...
        "星河大学学生创新协会",
        [
            surface("region-0001", "fragment-1", 0, "星河大学学生创新协会"),
            surface("region-0001", "fragment-1", 1, "Astra-Club"),
        ],
    )
    incoming = fragment("region-0002", "fragment-1", ["ASTRA CLUB", "创协"])
...
def test_one_region_plan_can_create_and_attach_together() -> None:
    existing = global_object(
        ...
        "global-000001-01",
        "创协",
        [surface("region-0001", "fragment-1", 0, "创协")],
    )
    first = fragment("region-0002", "fragment-1", ["学生创协"])
    ...
    assert {item.canonical_name for item in next_state.objects} == {"创协", "新生赛"}
    attached = next(
        item for item in next_state.objects if item.global_object_id == "global-existing"
    )
    assert [item.surface_form for item in attached.surface_atoms] == ["创协", "学生创协"]


def test_multiple_incoming_fragments_can_form_one_new_object() -> None:
    first = fragment("region-0001", "fragment-1", ["ASTRA CLUB"])
    second = fragment("region-0001", "fragment-2", ["校创协"])
...
def test_merge_keeps_earliest_existing_uuid() -> None:
    survivor = global_object(
        ...
        "global-000001-01",
        "星河计划",
        [surface("region-0001", "fragment-1", 0, "星河计划")],
    )
    duplicate = global_object(
        "global-second",
        "global-000002-01",
        "星河计划杯",
        [surface("region-0002", "fragment-1", 0, "星河计划杯")],
    )
    incoming_fragment = fragment("region-0003", "fragment-1", ["星河计划创意挑战赛"])
...
def test_case_c_reference_finalization_links_five_objects_without_span_replacement() -> None:
    ...
    reference_evidence = assertion(
        ...
        "claim-1",
        kind="reference",
        statement="创协主要品牌赛事的名称、比赛形式和基本定位集中记录于“品牌活动”表格。",
