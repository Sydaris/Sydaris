...
def _initial_turn(
    *,
    statement: str | None = "星河计划杯过去通常申请两个场地。",
...
def _fragment_turn() -> ModelTurn:
    return _json_turn(
        {
            "fragments": [
                {"fragment_key": "F1", "surface_forms": ["星河计划杯"]},
...
def _one_fragment_turn(
    *,
    surface_forms: Sequence[str] = ("星河计划杯",),
...
def _blocks(
    paragraph: str = "星河计划杯过去通常申请两个场地。大型比赛必须提前申请场地。",
) -> tuple[ParsedBlock, ...]:
    return (
        ParsedBlock(
...
async def test_compiles_three_direct_json_stages(tmp_path: Path) -> None:
    ...
    assert snapshot.object_fragments[0].surface_forms == ["星河计划杯"]
...
def test_fragment_can_extend_atomic_hint_with_source_local_reusable_name() -> None:
    ...
    claims = [_source_claim("claim-1", "创协成立于2000年。")]
    submission = ObjectFragmentSubmission(
        fragments=[
            ObjectFragmentDraft(
                fragment_key="F1",
    ...
    _validate_fragment_submission(
        ...
        claims,
        same_referent_drafts=[draft],
        source_blocks=_blocks("星河大学学生创新协会（ASTRA CLUB）。之后创协成立于2000年。"),
    )
...
async def test_incremental_review_does_not_duplicate_existing_claim(tmp_path: Path) -> None:
    duplicate = _json_turn(
        {
            "claims": [
                {
                    "statement_markdown": "星河计划杯过去通常申请两个场地。",
                    "supporting_block_ids": ["p0001-b0002"],
                    "context_dependent": False,
                }
            ]
    ...
    one_fragment = _json_turn(
        {
            "fragments": [{"fragment_key": "F1", "surface_forms": ["星河计划杯"]}],
            "assertions": [
                {
                    "claim_id": "claim-1",
                    "statement_template_markdown": ("{{fragment:F1}}过去通常申请两个场地。"),
...
def test_fragment_prompt_defines_leaf_ir_without_global_identity() -> None:
    ...
    assert "包含该主体 Fragment" in OBJECT_FRAGMENT_SYSTEM_PROMPT
    assert "不得加入仅作为归属背景" in OBJECT_FRAGMENT_SYSTEM_PROMPT
    assert "被明确呈现为集合主体" in OBJECT_FRAGMENT_SYSTEM_PROMPT
...
def test_case_c_reference_uses_semantic_links_without_object_mentions() -> None:
    event_names = ["星河计划", "四国大战", "萍水相逢", "会员大赛", "院系杯"]
    claim = _source_claim(
        "claim-1",
        "创协主要品牌赛事的名称、比赛形式和基本定位集中记录于“品牌活动”表格。",
        kind="reference",
    )
    submission = ObjectFragmentSubmission(
        fragments=[
...
def test_case_d_grounded_assertion_keeps_anchored_reference_validation() -> None:
    claim = _source_claim("claim-1", "星河计划是换届传承活动。")
    with pytest.raises(ValueError, match="grounded Assertion，不能使用 semantic links"):
        _validate_fragment_submission(
            ObjectFragmentSubmission(
                fragments=[ObjectFragmentDraft(fragment_key="F1", surface_forms=["星河计划"])],
                assertions=[
                    FragmentAssertionTemplateDraft(
                        claim_id="claim-1",
                        kind="grounded",
            ...
            [claim],
            source_blocks=_blocks("星河计划是换届传承活动。"),
        )
...
def test_fragment_template_does_not_require_reverse_rendering() -> None:
    claims = [
        _source_claim("claim-1", "星河计划杯通常申请两个场地。"),
    ]
    submission = ObjectFragmentSubmission(
        fragments=[ObjectFragmentDraft(fragment_key="F1", surface_forms=["星河计划杯"])],
        assertions=[
            FragmentAssertionTemplateDraft(
                claim_id="claim-1",
    ...
    _validate_fragment_submission(
        submission,
        claims,
        source_blocks=_blocks("星河计划杯通常申请两个场地。"),
    )
...
def test_fragment_rejects_self_identity_alias_collapse() -> None:
    claim = _source_claim("claim-1", "25-26学年创协会长为林明远，署于2026年春。")
    collapsed = ObjectFragmentSubmission(
        fragments=[
            ObjectFragmentDraft(
                fragment_key="F1",
                surface_forms=["25-26学年创协会长", "林明远"],
            )
        ],
        assertions=[
            FragmentAssertionTemplateDraft(
...
class BatchJsonModel:
    ...
    async def complete_turn(
        ...
        *,
        ...
        label = "星河计划杯" if node_id == "region-0002" else "会员大会"
