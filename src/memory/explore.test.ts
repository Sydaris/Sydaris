...
describe("searchMemory", () => {
  ...
  it("reuses Locate while bounding output and stripping SourceBlock excerpts", async () => {
    ...
    const result = await searchMemory("  星河计划  ", { onLocate });

    expect(retrieve).toHaveBeenCalledWith({ query: "星河计划", signal: undefined });
...
function followAssertionRows(includeReference = false) {
  ...
  const assertions = [{
    ...
    objectLinks: [objectLink("global-event", "星河计划")],
  }, {
    ...common,
  ...
  return includeReference ? [...assertions, {
    ...
    objectLinks: [],
    objectCoverage: [objectLink("global-event", "星河计划")],
  }] : assertions;
}
...
describe("followObject", () => {
  function useFollowDatabase(input: { includeTarget?: boolean; includeReference?: boolean } = {}) {
    const allObjects = [
      globalObject("global-event", "event-key", "星河计划", ["星河计划", "该活动"]),
  ...
  it("follows resolved Object relations and ranks by focus", async () => {
    ...
    expect(result.objects[0]).toMatchObject({ id: "global-event", canonicalName: "星河计划" });
    expect(result.assertions[0].renderedStatement).toBe("星河计划由学生会举办。");
    expect(JSON.stringify(result)).not.toContain("SOURCEBLOCK_MARKDOWN");
