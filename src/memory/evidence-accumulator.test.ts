...
function initial(): MemoryRetrievalResult {
  return {
    ...
    seedMap: {
      facets: [{ id: "facet-0", text: "星河计划", source: "query" }],
      objects: [{
        ref: "O1",
        id: "object-1",
...
describe("MemoryEvidenceAccumulator", () => {
  ...
  it("resolves a unique in-turn Object by O#、canonical name or surface form", () => {
    ...
    expect(accumulator.objectForModelReference("“星河计划”")).toEqual({
      id: "object-1",
      canonicalName: "星河计划",
    });
  });
  ...
  it("does not guess when an in-turn Object name is ambiguous", () => {
    ...
    expect(accumulator.objectForModelReference("星河计划")).toBeUndefined();
  });
