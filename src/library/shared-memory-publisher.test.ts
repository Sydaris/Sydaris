...
function resolvedObject(key: string, label = "星河计划杯"): GlobalObjectDraft {
  return {
...
describe("library Shared Brain publication preparation", () => {
  ...
  it("preserves a resolved Object identity when later material adds new cognition", () => {
    ...
    expect(initial.objects).toEqual([{ id: DRAFT_ID, canonicalName: "星河计划杯" }]);
    expect(later.objects).toEqual([{ id: DRAFT_ID, canonicalName: "星河计划杯" }]);
  ...
  it("publishes coarse Reference before a small number of grounded facts", () => {
    const publication = prepareSemanticPublication({
      ...
      assessment: {
        ...
        assertionCandidates: [{
          statement: "星河计划杯的报名截止时间为10月24日中午。",
