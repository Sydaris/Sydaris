...
1. 在 Plugin 项目的 `view/` 目录中定义 Schema、Commands 和 Events。
...
发布前应使用自己拥有的 npm scope，先发布 SDK，再发布 Plugin，
并用 tarball 做一次不依赖 registry 的发布前测试：
...
pnpm build
pnpm sydaris:plugin remove my.plugin --purge
```
